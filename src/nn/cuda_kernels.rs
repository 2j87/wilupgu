//! CUDA C kernel sources mirroring the semantics of the WGSL shaders in
//! `wilupgu/src/shaders/`. Each constant is NVRTC-compiled lazily (and cached)
//! the first time the matching `BuiltInShader` op runs on the CUDA backend.
//! Elementwise kernels use 256-thread 1D blocks; 2D-indexed kernels
//! (causal mask, RoPE, RoPE bwd) keep one thread per (token, channel) pair
//! exactly like their WGSL counterparts. MatMul/MatMulTrp/MatMulWeightBwd are
//! NOT here — those go through cuBLAS SGEMM (see `cuda_exec.rs`).
